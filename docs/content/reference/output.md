...
describes what users get. Pick a format with `-o`, or let hexpm choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
hexpm <command> -o table   # aligned columns for reading
hexpm <command> -o jsonl   # one JSON object per line, for piping
hexpm <command> -o json    # a single JSON array
hexpm <command> -o csv     # spreadsheet friendly
hexpm <command> -o tsv     # tab-separated
hexpm <command> -o url     # just the URL column
hexpm <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
hexpm <command> --fields id,title,url
```
...
```bash
hexpm <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
hexpm <command>            # a table, because this is a terminal
hexpm <command> | wc -l    # JSONL, because this is a pipe
```
